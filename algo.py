...
def predict_result(home_team, away_team, city):
    ...
    home_score = round(df[(df['home_team'] == home_team) & (df['away_team'] == away_team)]['home_score'].mean())
    away_score = round(df[(df['home_team'] == home_team) & (df['away_team'] == away_team)]['away_score'].mean())
    ...
    return f"Prédiction de résultat : {winner} gagne avec une probabilité de {win_percentage:.2f}%. Scores possibles : {away_score}-{home_score}."
